...
>    - **`clarify`** (`/workflow clarify <name>`): Ambiguity Checkpoint detecting omissions and conducting a 1-by-1 Socratic interview using `ask_question`, writing an incremental ADR in `.workflow/specs/active/<name>/adrs/0000_adr_<brief-decision-description>.md`.
...
> 3. **Deterministic 7-Stage Multi-Subagent Pipeline**: When triggering `/workflow run <spec>` (or with `--from <stage>` / `--only <stage>` / `--no-worktree`, freely combinable), the AI Agent MUST NOT assume stages ran automatically inside the python CLI. Instead, the AI Agent MUST:
>    - First run `uv run skills/workflow/scripts/workflow_runner.py run <spec> [flags]` to initialize and sync the physical worktree (`.workflow/worktrees/<spec>/worker/`). If `--no-worktree` was passed, no physical worktree or separate `feat/<spec>-worker` branch is created — all active stages run directly in the current working directory on the active branch (routed onto `feat/<spec>` in place if that branch is protected).
>    - Inspect the `Active Stages to Run` and `subagent_directives` returned by the CLI — with `--no-worktree`, the directives target the current working directory instead of `.workflow/worktrees/<spec>/worker/`.
...
>      5. **Stage 5 (Quality)**: `invoke_subagent(Subagents=[{'TypeName': 'workflow-quality-worker', 'Role': 'Quality Subagent', 'Prompt': 'Audit holistic quality gates (100/100, zero comments, OWASP clearance) in worktree.'}])`
>         * If Quality-Worker returns `NEEDS_FIX` or `NEEDS_REFACTOR`, loop back to that subagent (up to `max_revisions: 3`).
>      6. **Stage 6 (Doc)**: `invoke_subagent(Subagents=[{'TypeName': 'workflow-doc-worker', 'Role': 'Doc Subagent', 'Prompt': 'Author incremental ADRs (0000_adr_<description>.md), verify spec acceptance criteria, and synthesize canonical PR summary in worktree.'}])`
>      7. **Stage 7 (Git)**: `invoke_subagent(Subagents=[{'TypeName': 'workflow-git-worker', 'Role': 'Git Subagent', 'Prompt': 'Conduct interactive Grilling Session confirmation via ask_question with developer. Default Security: Commit locally and do NOT open PR on remote origin unless --pr flag is passed or explicitly authorized.'}])`
...
>    - **`--no-worktree` Exception**: When `/workflow run <spec> --no-worktree` is used, this hierarchical worktree/staging-branch separation is intentionally skipped — all 7 stages run directly inside the current working directory on the active branch (routed onto `feat/<spec-name>` in place only if the active branch is protected). `--auto-merge` is a no-op in this mode (there is no separate worker branch to fold back in), and `--pr`/`workflow pr` will open a PR from the active branch straight to the repository's default branch instead of from `feat/<spec-name>-worker` into `feat/<spec-name>`. Because there is no isolated checkout, per-stage checkpoint commits also include `.workflow/specs/`, `.workflow/memory/`, and `.workflow/prs/` bookkeeping changes alongside the code, unlike worktree mode where those files never enter the worker branch.
...
> 13. **Default Security Gate (Local Commits by Default)**: By default, all pipeline runs stop after the local commit inside the worktree. Autonomous subagents and CLI commands MUST NOT push to remote `origin` or open public PRs unless the explicit `--pr` flag was provided (e.g. `/workflow run <spec> --pr`) or the human developer explicitly authorizes PR creation during the interactive grilling session.
...
> 16. **Strict Skill Immutability & Zero Self-Modification**: The workflow skill and all agent skills under `.agents/` or `skills/` are strictly read-only execution engines. Autonomous subagents and code formatting tools (`formatter_manager.py`) MUST NEVER modify, format, or commit changes to `.agents/`, `skills/`, `.venv/`, or root framework tooling. Accidental modifications to `.agents/` are deterministically detected and reverted before commit creation.
...
| `/workflow run` | `workflow run <spec> [--only <st>] [--from <st>] [--dry-run] [--pr] [--no-worktree]` | **Primary Engine**: Run 7-stage subagent pipeline with auto-formatting & checkpoints |
| `/workflow pr` | `workflow pr <spec> [--title <title>] [--no-push]` | Create or update GitHub Pull Request targeting `feat/<spec>` with canonical summary |
