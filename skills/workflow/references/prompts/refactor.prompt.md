...
3. **Cycle Inspection & Code Health Audit**:
   - Inspect `.workflow/specs/active/<spec>/issues/` for pending refactoring tasks or codebase areas requiring cleanup.
...
6. **Heartbeat & Spec-Scoped ADRs**:
   - Record refactoring decisions in `.workflow/specs/active/<spec>/adrs/` and update heartbeat in `.workflow/daemons.json`.
